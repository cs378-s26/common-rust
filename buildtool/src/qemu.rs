...
pub fn run(kvm: bool, cores: u8, mem_g: u8, release: bool, target: Target, disk_path: String) -> Result<()> {
    ...
    if !disk_path_absolute.exists() {
        return Err(anyhow!("Disk path '{}' does not exist, try creating one with qemu-img create -f raw disk.img 64M", disk_path_absolute.display()));
    }
