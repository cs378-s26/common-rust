...
To avoid inserting architecture-specific code into the core virtual memory management logic, we call architecture-specific functions for allocating and freeing physical frames, modifying page table entries, flushing the TLB, and issuing shootdowns. However, the latter is not actually needed on ARM, and some of the interfaces
