...
To trigger an event, call `push_event(event: Event, core: CoreId, should_alloc: bool)`, which will push the event onto the specified core's event queue. Important: See [Allocation](#Allocation) for a discussion of when to use `should_alloc`.
