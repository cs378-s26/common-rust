...
kernel_common::integration_test!({
    ...
    use kernel_common::{
        ...
        process::Process,
        sync::MutexLike,
        syscall::{SyscallContext, syscall_handler},
    ...
    impl SyscallContext for TestSyscallContext {
        ...
        fn is_user_address(&self, _ptr: u64) -> bool {
            true
        } // Treat all as user for test
    }
    ...
    let ext2 = Ext2::new_from_block_devices(&mut block_devices).expect("ext2 filesystem not found");
    drop(block_devices);
    ...
    Process::run(process.clone(), move || {
        ...
        use kernel_common::syscall::{AT_FDCWD, number};
        ...
        let hello_read = call_syscall(
            &thread,
            number::READ,
            fd_hello,
            hello_buf.as_mut_ptr() as u64,
            32,
            0,
        );
        assert!(hello_read > 0);
        kprintln!(
            "7. open/read existing file passed: {}",
            core::str::from_utf8(&hello_buf[..hello_read as usize]).unwrap()
        );
        call_syscall(&thread, number::CLOSE, fd_hello, 0, 0, 0);

        // 4. Test directory fd and relative path
        let dir_path = "dir\0";
        let fd_dir = call_syscall(
            &thread,
            number::OPENAT,
            AT_FDCWD as u64,
        ...
        let fd_nested = call_syscall(
            ...
            number::OPENAT,
            fd_dir,
            nested_path.as_ptr() as u64,
            0,
            0,
        );
        assert!(fd_nested >= 3);
        let mut nested_buf = [0u8; 32];
        let nested_read = call_syscall(
            &thread,
            number::READ,
            fd_nested,
        ...
        let fd_new_rel = call_syscall(
            &thread,
            number::OPENAT,
            fd_dir,
            new_rel_path.as_ptr() as u64,
            64,
            0,
        );
        assert!(fd_new_rel >= 3);
        ...
        let res_no_file = call_syscall(
            &thread,
            number::OPENAT,
            AT_FDCWD as u64,
            no_file_path.as_ptr() as u64,
            0,
            0,
        );
