...
static void test_basic_threading() {
  ...
  TEST_ASSERT_MSG(counter.get() == NUM_THREADS, "Not all threads completed");
...
static void test_yield() {
  ...
  TEST_ASSERT_MSG(yield_count.get() == NUM_THREADS, "Not all threads completed after yield");
...
static void test_multicore_work() {
  ...
  TEST_ASSERT_MSG(total_work == NUM_THREADS, "Total work doesn't match thread count");

  // With multiple cores, at least 2 should have done work
  if (Sys::core_count > 1) {
    TEST_ASSERT_MSG(active_cores >= 2, "Work not distributed across cores");
  }
