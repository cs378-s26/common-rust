...
static void test_all_cores_participate() {
    ...
    TEST_ASSERT_MSG(participating == Sys::core_count,
                    "Not all cores participated");
...
static void test_thread_migration() {
    ...
    if (Sys::core_count > 1) {
        TEST_ASSERT_MSG(migration_count > 0,
                        "No thread migrations detected with multiple cores");
    }
...
static void test_percore_isolation() {
    ...
    TEST_ASSERT_MSG(total == INCREMENTS,
                    "Total count mismatch - race condition?");
...
static void test_stress() {
    ...
    TEST_ASSERT_MSG(completed.get() == NUM_THREADS,
                    "Not all threads completed in stress test");
