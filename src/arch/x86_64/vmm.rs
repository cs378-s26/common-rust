...
pub fn vmap(space: u64, vaddr: u64, paddr: u64, options: PagingOptions) {
    ...
    let pframe = PhysFrame::<Size4KiB>::from_start_address(PhysAddr::new(paddr)).unwrap_or_else(|_| panic!("misaligned physical address {:x} to vmap", paddr));
    let toilet = {
...
pub fn vunmap(space: u64, vaddr: u64) -> Option<u64> {
    ...
    let vpage = Page::<Size4KiB>::from_start_address(VirtAddr::new(vaddr)).unwrap_or_else(|_| panic!("misaligned virtual address {:x} to vunmap", vaddr));
