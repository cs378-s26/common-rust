use crate::devices::device_discovery::{DeviceType, DeviceDiscovery, DeviceDriver, DeviceNode};
            ...
            if let Some(c) = node.compatible() {
                if c.all().any(|s| s == "arm,pl011") {
                    if let Some(reg) = node.reg().and_then(|mut r| r.next()) {
                        ...
                        return Some(Box::new(UartPl011Driver { phys_address, virt_mapping: None }));
                    }
                }
            }
