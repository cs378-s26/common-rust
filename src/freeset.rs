...
impl FreeSet {
    ...
    pub fn remove_range_by_length(&mut self, length: usize) -> Result<usize, &'static str> {
        let Some(free) = self.length_set.lower_bound(Included(&length)).get() else {
            return Err("there is no free range big enough");
        };
        let whole_base = free.base;
        let whole_length = free.length;
        self.remove_range_by_base_unchecked(whole_base, length, whole_base, whole_length)
            .map(|_| whole_base)
    }
}
