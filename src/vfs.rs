...
pub struct VFS {
    filesystems: Mutex<BTreeMap<usize, Arc<dyn Filesystem>>>,
    inode_cache: Mutex<BTreeMap<usize, BTreeMap<usize, Arc<dyn INode>>>>,
...
impl VFS {
    pub fn mount(&self, filesystem: Arc<dyn Filesystem>) -> usize {
        ...
        let filesystem_id = self.filesystem_id_counter.fetch_add(1, Ordering::Relaxed);
